...
def _format_lang_param(language):
    if not language:
        return "en-US"
    if "-" in language:
        return language
...
def _parse_google_response(text):
    transcripts = []
    for line in text.splitlines():
        ...
        if not isinstance(obj, dict):
            continue
        results = obj.get("result") or obj.get("results") or []
        for res in results:
            alternatives = res.get("alternative") or res.get("alternatives") or []
...
def process_chunk(chunk_index, start, chunk_duration, original_file, language):
    ...
    actual_start = max(0, start - CHUNK_OVERLAP)
    actual_duration = chunk_duration + (CHUNK_OVERLAP if start > 0 else 0) + SILENCE_PADDING
    text_result = ""
    try:
        cmd = [
            'ffmpeg', '-y',
        ...
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=REQUEST_TIMEOUT)
        if os.path.exists(temp_chunk_file) and os.path.getsize(temp_chunk_file) > 100:
            with open(temp_chunk_file, 'rb') as f:
                audio_bytes = f.read()
            lang_param = _format_lang_param(language)
            url = f"https://www.google.com/speech-api/v2/recognize?client=chromium&lang={lang_param}"
            headers = {
                "User-Agent": "Mozilla/5.0",
                "Content-Type": "audio/x-flac; rate=16000"
            }
            try:
                r = requests.post(url, headers=headers, data=audio_bytes, timeout=REQUEST_TIMEOUT)
                if r.status_code == 200 and r.text:
                    parsed = _parse_google_response(r.text)
                    if parsed:
                        text_result = parsed
            except Exception as e:
                logging.warning(f"Google STT request failed for chunk {chunk_index}: {e}")
    except Exception as e:
...
def handle_media(message):
    ...
    if getattr(media, 'file_size', 0) > MAX_UPLOAD_SIZE:
        ...
        return
    file_type = "Unknown"
    if message.voice: file_type = "Voice"
    elif message.audio: file_type = "Audio File"
    elif message.video: file_type = "Video"
    elif message.document: file_type = f"Document ({message.document.mime_type})"
