...
def parse_google_speech_response(text):
    transcripts = []
    ...
    for line in lines:
        ...
        try:
            ...
            results = data.get("result") or data.get("results") or data.get("response")
            if results:
                if isinstance(results, list):
                    for r in results:
                        alts = r.get("alternative") or r.get("alternatives")
                        if alts and isinstance(alts, list):
                            best = alts[0].get("transcript")
                            if best:
                                transcripts.append(best)
                elif isinstance(results, dict):
                    alts = results.get("alternative") or results.get("alternatives")
                    if alts and isinstance(alts, list):
                        best = alts[0].get("transcript")
                        if best:
                            transcripts.append(best)
    ...
    matches = re.findall(r'"transcript"\s*:\s*"([^"]+)"', text)
    if matches:
        return " ".join(matches)
    return text

def upload_and_transcribe_google(file_path, language=None):
    url = f"https://www.google.com/speech-api/v2/recognize?client=chromium&lang={language or 'en'}"
    headers = {
    ...
    r.raise_for_status()
    return parse_google_speech_response(r.text)
...
def lang_cb(call):
    ...
    if origin == "file":
        ...
        try:
            text = upload_and_transcribe_google(file_path, language=code)
            if not text:
...
def handle_media(message):
    ...
    try:
        ...
        if not lang:
            ...
            return
        text = upload_and_transcribe_google(file_path, language=lang)
        if not text:
