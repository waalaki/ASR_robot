...
VOSK_ZIP_URL = os.environ.get("VOSK_ZIP_URL", "https://alphacephei.com/vosk/models/vosk-model-en-us-0.22-lgraph.zip")
...
def ensure_vosk_model():
    if os.path.isdir(VOSK_MODEL_PATH):
        return
    zip_dest = os.path.join(DOWNLOADS_DIR, "vosk_model.zip")
    urllib.request.urlretrieve(VOSK_ZIP_URL, zip_dest)
...
def process_chunk(chunk_index, start, chunk_duration, original_file, language):
    temp_chunk_file = os.path.join(DOWNLOADS_DIR, f"temp_{os.path.basename(original_file)}_{chunk_index}.wav")
    r_local = sr.Recognizer()
    text_result = ""
    try:
        cmd = [
            'ffmpeg', '-y', '-ss', str(start), '-t', str(chunk_duration),
            '-i', original_file, '-vn', '-ar', '16000', '-ac', '1', '-f', 'wav', temp_chunk_file
        ]
        subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if os.path.exists(temp_chunk_file) and os.path.getsize(temp_chunk_file) > 100:
            if vosk_model is not None and language == "en" and chunk_duration <= CHUNK_SECONDS:
                try:
                    text_result = transcribe_with_vosk(temp_chunk_file)
                except Exception as e:
                    logging.warning(f"Vosk error chunk {chunk_index}: {e}")
                    text_result = ""
            else:
