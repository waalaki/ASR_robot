...
def transcribe_with_gemini(file_path, mime_type, language=None):
    if not gemini_rotator.keys:
        raise RuntimeError("GEMINI_KEY(s) not configured")
...
def lang_cb(call):
    ...
    try:
        text = transcribe_with_gemini(file_path, mime_type, language=code)
        if not text:
...
def handle_media(message):
    ...
    try:
        ...
        text = transcribe_with_gemini(dest_path, mime_type, language=lang)
        if not text:
