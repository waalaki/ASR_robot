...
def google_speech_recognize(file_path, language, api_key):
    if not api_key:
        return ""
    try:
        ...
        config = {"encoding":"FLAC","sampleRateHertz":16000,"languageCode":language or "en-US","enableAutomaticPunctuation":True}
        ...
        for r in results:
            ...
            if alts:
                transcripts.append(alts[0].get("transcript", "").strip())
        return " ".join(transcripts).strip()
    except Exception as e:
        logging.warning(f"Google speech error: {e}")
        return ""
...
def process_chunk(chunk_index, start, chunk_duration, original_file, language):
    ...
    actual_duration = chunk_duration + (CHUNK_OVERLAP if start > 0 else 0)
    text_result = ""
    try:
        cmd = [
            'ffmpeg', '-y',
        ...
        if os.path.exists(temp_chunk_file) and os.path.getsize(temp_chunk_file) > 100:
            text_result = google_speech_recognize(temp_chunk_file, language, GOOGLE_SPEECH_KEY)
    except Exception as e:
