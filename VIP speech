...
def transcribe_file(file_path, language=None):
    ...
    for i in range(total_chunks):
        start = i * chunk_sec
        with tempfile.NamedTemporaryFile(delete=False, suffix=f'_{i}.flac', dir=DOWNLOADS_DIR) as tf:
            tmp_flac = tf.name
        ...
        if os.path.getsize(tmp_flac) < 100:
            os.remove(tmp_flac)
            continue

        with sr.AudioFile(tmp_flac) as source:
            audio_data = r.record(source)
        try:
            if language:
                part = r.recognize_google(audio_data, language=language)
            else:
                part = r.recognize_google(audio_data)
            texts.append(part)
